...
@edgedata """ + self.filename + '-edge.rn'
    ...
    def rpg_generate_data_set(self, rpg, secret):
        soc_node = rpg.soc_node
        soc_edge = rpg.soc_edge
        yes_no = {True: 'yes', False: 'no'}
        # write soc nodes
        soc2write = [{'UID': node,
                      'Secret': yes_no[self.rpg.attr_net.has_edge(node, secret)]}
...
@edgedata """ + self.filename + '-edge.rn'
    ...
    def generate_test_set(self, test_set, secret):
        node2test = []
        yes_no = {True: 'yes', False: 'no'}
        for node in test_set:
            node2test.append(','.join([node, yes_no[self.rpg.attr_net.has_edge(node, secret)]]))
        with open(os.path.join(self.path, self.filename + '.test'), 'w') as fp:
            for line in node2test:
                fp.write(line+'\n')
    ...
    def cross_validation(self, k=5, r=0.1, classifier='wvrn'):
        self.execute('-rclassifier %s -runs %d -sample %f -output %s %s.arff' % (classifier,
                                                                                 k,
                                                                                 r,
    ...
    def run_test(self, test_set, secret, classifier='wvrn'):
        self.generate_test_set(test_set, secret)
        self.execute('-rclassifier %s -lclassifier null -test %s.test -output %s %s.arff' % (classifier,
                                                                           os.path.join(self.path, self.filename),
                                                                           os.path.join(self.path, self.filename),
                                                                           os.path.join(self.path, self.filename)))
        with open(os.path.join(self.path, self.filename + '.predict'), 'rb') as fp:
