...
def July10_01():
    with open('data/exp_1684_attr_six.txt', 'r') as fp:
        ...
        plt.axis([0.35, 0.95, 0.15, 0.45])
        plt.xlabel(r'$\theta$', fontsize=20)
        plt.ylabel(r'Percentage of Masked Attributes', fontsize=20)
        plt.xticks(np.arange(0.35, 1.0, 0.1), fontsize=18)
        plt.yticks(np.arange(0.15, 0.46, 0.05), fontsize=18)
        plt.legend(lines,
                    ('Random Mask', 'd-KP(Greedy)', 'd-KP(DP)', 'EPPD', 'NB'), fontsize=18, loc='upper right', ncol=2)
        plt.grid(True)
        plt.tight_layout()
        # plt.show()
...
def July22_01_bar():
    ...
    xx = ('Random Mask', 'd-KP(Greedy)', 'd-KP(DP)', 'EPPD', 'NB')
...
def July25_final():
    with open('data/exp_3437e_attr_4.txt', 'r') as fp:
        ...
        lines = plt.plot(x, y, linewidth=2)
        plt.setp(lines[0], marker='o', color=scalarMap.to_rgba(1), markersize=8, linewidth=2.0)
        plt.setp(lines[1], marker='s', color=scalarMap.to_rgba(2), markersize=8, linewidth=2.0)
        plt.setp(lines[2], marker='>', color=scalarMap.to_rgba(3), markersize=8, linewidth=2.0)
        # plt.setp(lines[3], marker='s', color=scalarMap.to_rgba(4), markersize=8, linewidth=2.0)
        # plt.setp(lines[4], marker='d', color=scalarMap.to_rgba(5), markersize=8, linewidth=2.0)
        # plt.setp(lines[5], marker='>', color=scalarMap.to_rgba(6), markersize=8, linewidth=2.0)
        ...
        plt.legend(lines,
                    ('Random Mask', 'EPPD', 'NB'), fontsize=18, loc='lower left', ncol=1)
        plt.grid(True)
        plt.tight_layout()
        # plt.show()
