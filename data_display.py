...
def July10_01():
    with open('out/exp_1684_score_unique_six.txt', 'r') as fp:
        ...
        plt.axis([0.35, 0.95, 0.5, 0.77])
        plt.xlabel(r'$\theta$', fontsize=20)
        plt.ylabel(r'Normalized Uniqueness Score', fontsize=20)
        plt.xticks(np.arange(0.35, 1.0, 0.1), fontsize=18)
        plt.yticks(np.arange(0.5, 0.77, 0.05), fontsize=18)
        plt.legend(lines,
                    ('Random Mask', 'd-KP(Greedy)', 'd-KP(DP)', 'EPPD', 'DP', 'NB'), fontsize=18, loc='lower right', ncol=2)
        plt.grid(True)
        plt.tight_layout()
        # plt.show()
...
def July22_01_bar():
    ...
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cm)
    y0 = [[0.57380525686977313, 0.5086618876941458, 0.5086618876941458, 0.5038829151732378, 0.5232974910394266, 0.559173450219],
          [0.33484400101462752, 0.23057411008708886, 0.22178067134522703, 0.23522448634480425, 0.24782277838843325, 0.30976225725],
          [0.43027522935779816, 0.32360300250208507, 0.32068390325271057, 0.32068390325271057, 0.3573811509591326, 0.411942881869],
          [0.34693486590038314, 0.22924648786717752, 0.21839080459770116, 0.24521072796934865, 0.26500638569604085, 0.308707124011],
          [0.32412412412412406, 0.2702702702702703, 0.2702702702702703, 0.26326326326326327, 0.28928928928928926, 0.292887029289],
          [0.410137810866466, 0.353397750673214, 0.34579439252336447, 0.33914145414224617, 0.3592586725803897, 0.381875414182],
          [0.373729233820977, 0.2793206050086784, 0.2788246962558889, 0.2788246962558889, 0.3000247954376395, 0.351672198111],
          [0.2779732582688248, 0.24466338259441708, 0.24466338259441708, 0.2437250762373915, 0.2531081398076472, 0.270978600625]]
    y = np.matrix([[1 - i for i in j] for j in y0])
    y = y.transpose().tolist()
    x = ['#0', '#107', '#348', '#414', '#686', '#1684', '#1912', '#3437']
    xx = ('Random Mask', 'd-KP(Greedy)', 'd-KP(DP)', 'EPPD', 'DP', 'NB')
...
def July25_final():
    with open('out/exp_3437e_score_4j.txt', 'r') as fp:
        x = [float(i) for i in fp.readline().strip().split()]
        fp.readline()
        li = fp.readline()
        y = []
        ...
        # plt.setp(lines[4], marker='d', color=scalarMap.to_rgba(5), markersize=8, linewidth=2.0)
        # plt.setp(lines[5], marker='>', color=scalarMap.to_rgba(6), markersize=8, linewidth=2.0)
        plt.axis([0.2, 0.9, 0.6, 0.9])
        plt.xlabel(r'$\theta$', fontsize=20)
        plt.ylabel(r'Normalized Jaccard Score', fontsize=20)
        plt.xticks(np.arange(0.2, 1.0, 0.1), fontsize=18)
        plt.yticks(np.arange(0.6, 0.9, 0.05), fontsize=18)
        plt.legend(lines,
                    ('Random Mask', 'd-KP(Greedy)', 'EPPD', 'NB'), fontsize=18, loc='upper left', ncol=2)
        plt.grid(True)
        plt.tight_layout()
        # plt.show()
        plt.savefig("3437JScore.eps", format="eps")
